...
<template>
  <div class="w-full max-w-2xl bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl space-y-8">
    <div class="space-y-2">
      ...
      <p class="text-gray-400">All data is stored in the URL. No database needed.</p>
    </div>

    <!-- Basic Info -->
    <div class="space-y-4">
      <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Basic Info</h3>
      <div class="grid grid-cols-1 gap-4">
        ...
        <div>
          ...
          <textarea v-model="profile.bio" placeholder="Digital Nomad & Creator" rows="2"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-400 mb-1">Avatar URL (Optional)</label>
          <input v-model="profile.avatar" type="text" placeholder="https://..." 
                 class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" />
        </div>
      </div>
    </div>
    ...
    <!-- Socials -->
    <div class="space-y-4">
      <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Social Presence</h3>
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Links</h3>
      ...
      <div class="space-y-3">
        <div v-for="(link, index) in profile.links" :key="index" class="flex items-start gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
          <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
             <input v-model="link.title" type="text" placeholder="Link Title" class="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
    ...
    <div v-if="generatedUrl" class="pt-6 border-t border-white/10 animate-in fade-in zoom-in duration-500">
      <label class="block text-sm font-medium text-indigo-300 mb-2">Your Generated Link</label>
      ...
      <p class="mt-2 text-xs text-center text-gray-500">Note: This URL can be long. Use a link shortener if needed!</p>
    </div>
  </div>
</template>
